      ...
      input_descriptors : [{
        ...
        constraints : {
          fields: [{
            ...
            filter : {
              type  : 'string',
              const : "THIS IS MY ID AND IT'S UNIQUE LIKE A SNOWFLAKE"
            }
          }]
        }
      }]
...
export const myOfferingsApi: OfferingsApi = {
  async getOffering() { return myOffering },
  async getOfferings() { return [myOffering] }
}

const myExchangesApi: ExchangesApi = {
...
const api = new TbdexHttpServer({ 
  exchangesApi: myExchangesApi,
  offeringsApi: myOfferingsApi
});
